...
export default function LeaderboardPage() {
  ...
  const [weeklyWinners, setWeeklyWinners] = useState<Winner[]>([
    ...
    {
      rank: 8,
      name: "cheese_commander",
      address: "0xffff...1111",
      winnings: "4.8 VMF",
    },
    {
      rank: 9,
      name: "pepperoni_power",
      address: "0x2222...4444",
      winnings: "4.1 VMF",
    },
    {
  ...
  const referralCode = connection?.address
    ? `PIZZA-${connection.address.slice(2, 8).toUpperCase()}`
    : "PIZZA-XXXXXX";
  const referralLink =
    typeof window !== "undefined"
      ? `${window.location.origin}/game?ref=${referralCode}`
      : `/game?ref=${referralCode}`;
  ...
  const fetchFarcasterProfile = async (
    address: string
  ): Promise<{ username?: string; pfp?: string }> => {
    try {
      // Try to fetch from Farcaster API
      const response = await fetch(
        `https://api.farcaster.xyz/v2/users?fid=${address}`
      );
      if (response.ok) {
      ...
      if (profile.username || profile.pfp) {
        ...
        if (typeof window !== "undefined") {
          localStorage.setItem(
            `farcaster_profile_${winner.address}`,
  ...
  const renderWinnerEntry = (winner: Winner) => (
    <div
      key={winner.rank}
      className={`flex items-center justify-between p-3 rounded-xl border-2 transition-all duration-200 ${getRankColor(
        winner.rank
      )}`}
    >
      ...
      <div className="text-right">
        <p
          className={`font-bold ${
            winner.rank <= 3 ? "text-white" : "text-green-600"
          }`}
      ...
      <div className="max-w-4xl mx-auto">
              ...
              <div className="text-center">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                  <div className="bg-white p-3 rounded-lg border border-green-200">
                    ...
                    <p className="text-green-600" style={customFontStyle}>
                      8 players randomly selected every day at 12pm PST
                    </p>
                  </div>
                  <div className="bg-white p-3 rounded-lg border border-blue-200">
                    ...
                    <p className="text-blue-600" style={customFontStyle}>
                      10 random players selected with weighted probability based
                      on claimed toppings
                    </p>
                  </div>
                  <div className="bg-white p-3 rounded-lg border border-purple-200">
                    ...
                    <p className="text-purple-600" style={customFontStyle}>
                      Play daily, refer friends, and hold VMF tokens!
                    </p>
                  </div>
                </div>
              </div>
        ...
        <Dialog open={showWalletModal} onOpenChange={setShowWalletModal}>
          <DialogContent className="max-w-md mx-auto bg-white border-2 border-red-500 rounded-3xl">
            <DialogHeader>
              <DialogTitle
                className="text-xl sm:text-2xl text-red-800 flex items-center gap-2 justify-center"
                style={customFontStyle}
              >
                🍕 Connect Your Wallet 🍕
              </DialogTitle>
            ...
            <div className="space-y-4 p-4">
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <span
                          ...
                          style={{
                            display: wallet.iconImage ? "none" : "block",
                          }}
                        >
                          {wallet.icon}
                        </span>
              ...
              <div className="bg-blue-50 p-4 rounded-xl border border-gray-300">
                <h3
                  className="text-lg font-bold text-blue-800 mb-2 flex items-center gap-2"
                  style={customFontStyle}
                >
